...
void BMI160_Init() {
    I2C_Init(I2C_A_BASE);
    uint8_t read_val = BMI160_ReadRegister(BMI160_CMD_ADDR);
    BMI160_WriteRegister(BMI160_CMD_ADDR, read_val | 0x11); // this is for the accelerometer
    SysCtlDelay(1000);
    BMI160_WriteRegister(BMI160_CMD_ADDR, 0x15);
    //read_val = BMI160_ReadRegister(BMI160_CMD_ADDR); // call it again to resume the I2C line for the gyro
    //BMI160_WriteRegister(BMI160_CMD_ADDR, read_val | 0x15); // this is for the gyroscope
}
...
void BMI160_WriteRegister(uint8_t addr, uint8_t data) {
    ...
    bytes[0] = addr;
    bytes[1] = data;
    I2C_WriteMultiple(I2C1_BASE, BMI160_ADDR, bytes, 2);
}

uint8_t BMI160_ReadRegister(uint8_t addr) {
    uint8_t data[1];
    data[0] = addr;
    I2C_ReadMultiple(I2C1_BASE, BMI160_ADDR, data, 1); // Read 1 byte into 'data'
    return data[0];
}
...
void BMI160_MultiReadRegister(uint8_t addr, uint8_t* data, uint8_t num_bytes) {
    ...
    bytes[0] = addr;
    I2C_ReadMultiple(I2C1_BASE, BMI160_ADDR, bytes, num_bytes); // Send the register address to read from
    data = bytes;
}
